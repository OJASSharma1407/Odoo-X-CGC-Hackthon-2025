...
const SignupPage = ({ onSwitchToLogin }) => {
  ...
  const handleSignup = async () => {
    ...
    setIsLoading(true);
    try {
      const res = await fetch("http://localhost:5000/userAuth/sign-in", {
        method: "POST",
      ...
      <Card className="w-full max-w-md z-10">
        <div className="p-8">
          <div className="text-center mb-8">
            ...
            <h1 className="text-3xl font-bold bg-gradient-to-r from-emerald-600 to-blue-600 bg-clip-text text-transparent mb-2">QuickDesk</h1>
          ...
          <form className="space-y-5">
            ...
            <div>
              ...
              <Input icon={LockIcon} type="password" placeholder="Create a password" value={password} onChange={(e) => setPassword(e.target.value)} required />
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Confirm Password</label>
              <Input icon={LockIcon} type="password" placeholder="Confirm your password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} required />
            </div>
          ...
          <p className="text-center text-sm text-gray-600 mt-6">
            ...
            <NavLink to='/login'>
              <button type="submit" className="text-emerald-600 hover:text-emerald-700 font-semibold hover:underline transition-colors">
                Sign in
              </button>
            </NavLink>
          </p>
